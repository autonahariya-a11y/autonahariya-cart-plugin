...
// Hide old cart
$('div.cart.special_cart_with_upgrades').hide();
// Build drawer HTML
...
function open(){refresh();$('#anD').addClass('op');$('#anO').addClass('v');$('body').css('overflow','hidden')}
function close(){$('#anD').removeClass('op');$('#anO').removeClass('v');$('body').css('overflow','')}
// Events
$(document).on('click','#anX,#anO',close);
$(document).on('click','#anCont',function(e){e.preventDefault();close()});
$(document).on('keydown',function(e){if((e.key==='Escape'||e.keyCode===27)&&$('#anD').hasClass('op'))close()});
// Cart icon
$(document).on('click','#link_order_with_counter a, ul#header_cart_nav a.cart, span.cart_with_items_text, span.cart_with_items_counter',function(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();if($('#anD').hasClass('op'))close();else open();return false});
// Add to cart
function addFromPage(){
  ...
  // Get the correct price — try sale price first, then regular
...
function afterAdd(){addFromPage();setTimeout(open,500);setTimeout(function(){addFromPage();refresh()},1200)}
// Direct click on add-to-cart button
...
// Also watch for .clicked class being added to commit_to_real (Konimbo adds this on success)
...
// Qty
$(document).on('click','.qp',function(){var id=$(this).data('id'),c=load();if(c[id]){c[id].q++;save(c);refresh()}});
$(document).on('click','.qm',function(){var id=$(this).data('id'),c=load();if(c[id]&&c[id].q>1){c[id].q--;save(c);refresh()}});
$(document).on('click','.dl',function(){var id=$(this).data('id');$('.anD-it[data-id="'+id+'"]').addClass('rm');setTimeout(function(){var c=load();delete c[id];save(c);refresh()},400)});
// Checkout
$(document).on('click','#anGo',function(){if(Object.keys(load()).length)location.href='https://secure.konimbo.co.il/orders/autonahariya/new'});
// Cross-sell
$(document).on('click','.csi button:not(.ad)',function(){var ci=$(this).data('ci'),p=CS[ci];if(!p)return;var id='cs_'+ci,c=load();if(c[id]){c[id].q++}else{c[id]={t:p.n,p:p.p,q:1,i:'',u:'#'}};save(c);$(this).text('✓ נוסף!').addClass('ad');refresh()});
// Timer
